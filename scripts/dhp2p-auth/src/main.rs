...
struct Cli {
    ...
    #[arg(short, long)]
    relay: bool,
    /// Device username (for firmware that authenticates the P2P channel)
    #[arg(short, long, default_value = "admin")]
    username: String,
    /// Device password (for firmware that authenticates the P2P channel)
    #[arg(short = 'w', long)]
...
async fn main() {
    ...
    let (socket, session) =
        p2p_handshake(socket, serial, args.relay, args.username, args.password).await;
