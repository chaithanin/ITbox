...
export function LicenseImportUploader() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = body && typeof body === "object" && "message" in body && typeof body.message === "string"
          ? body.message : "การนำเข้าล้มเหลว / Import failed";
        setErrorMessage(msg); return;
      }
      setResult(body as ImportResult);
    } catch {
      setErrorMessage("การนำเข้าล้มเหลว กรุณาลองใหม่");
  ...
  function downloadErrorReport() {
    ...
    const lines = [
      ["row", "softwareName", "error"].map(csvEscape).join(","),
      ...result.errors.map((e) => [csvEscape(e.row), csvEscape(e.softwareName), csvEscape(e.error)].join(",")),
    ];
    const blob = new Blob(["﻿" + lines.join("\r\n") + "\r\n"], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = "license-error-report.csv";
      ...
      {result && (
        <div className="space-y-4">
          <div className="rounded-md border bg-muted/30 p-4 text-sm">
            ...
            <p className="mt-1">สร้างสำเร็จ / Created: <span className="font-semibold text-primary">{result.created}</span> ลิขสิทธิ์</p>
            <p>ข้าม / Skipped: <span className={result.failed > 0 ? "font-semibold text-destructive" : "font-semibold"}>{result.failed}</span> แถว</p>
          </div>
            ...
            <div className="space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <Button type="button" variant="outline" size="sm" onClick={downloadErrorReport}>
                  <Download className="h-4 w-4" /> ดาวน์โหลดรายงานข้อผิดพลาด
                </Button>
              </div>
              <Table>
                <TableHeader>
                  <TableRow><TableHead>แถว</TableHead><TableHead>Software</TableHead><TableHead>ข้อผิดพลาด</TableHead></TableRow>
                </TableHeader>
                <TableBody>
                  {result.errors.map((e, i) => (
                    <TableRow key={`${e.row}-${i}`}>
                      <TableCell>{e.row}</TableCell>
                      <TableCell>{e.softwareName || "-"}</TableCell>
          ...
          {result.created > 0 && (
            <Button variant="outline" asChild><Link href="/licenses">ไปที่ลิขสิทธิ์ / Go to Licenses</Link></Button>
          )}
        </div>
      )}
    </div>
