...
export function MaintenanceImportUploader() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch("/api/maintenance/import", { method: "POST", body: formData });
  ...
  function downloadErrorReport() {
    ...
    const lines = [
      ["row", "reference", "error"].map(csvEscape).join(","),
      ...result.errors.map((e) =>
        [csvEscape(e.row), csvEscape(e.reference), csvEscape(e.error)].join(",")
      ),
    ];
    ...
    a.href = url;
    a.download = "maintenance-error-report.csv";
      ...
      {result && (
        <div className="space-y-4">
          <div className="rounded-md border bg-muted/30 p-4 text-sm">
            ...
            <p className="mt-1">
              สร้างสำเร็จ / Created:{" "}
              <span className="font-semibold text-primary">{result.created}</span> รายการ / tickets
            </p>
            <p>
              ข้าม / Skipped:{" "}
            ...
            <div className="space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-sm font-medium">
                  ...
                  {result.failed > result.errors.length ? ` จาก / of ${result.failed}` : ""})
                </p>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>อ้างอิง / Reference</TableHead>
          ...
          {result.created > 0 && (
            <Button variant="outline" asChild>
              <Link href="/maintenance">ไปที่งานแจ้งซ่อม / Go to Maintenance</Link>
            </Button>
          )}
        </div>
      )}
